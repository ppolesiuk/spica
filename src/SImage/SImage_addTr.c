  ...
  const SImage_t     *src)
{
  ...
  case SFmt_Invalid:
    return;
  case SFmt_Gray:
    addTrGray(tgt, tgt->data_gray, src, SImage_subpixelGray, tr, tr_inv);
    return;
  case SFmt_RGB:
    addTrRGB(tgt, tgt->data_rgb, src, SImage_subpixelRGB, tr, tr_inv);
    return;
  case SFmt_SeparateRGB:
    addTrGray(tgt, SImage_dataRed(tgt),
      src, SImage_subpixelRed,   tr, tr_inv);
    addTrGray(tgt, SImage_dataGreen(tgt),
      src, SImage_subpixelGreen, tr, tr_inv);
    addTrGray(tgt, SImage_dataBlue(tgt),
