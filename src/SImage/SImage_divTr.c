  ...
  const SImage_t     *src)
{
  ...
  case SFmt_Invalid:
    return;
  case SFmt_Gray:
    divTrGray(tgt, tgt->data_gray, src, SImage_subpixelGray, tr, tr_inv);
    return;
  case SFmt_RGB:
    divTrRGB(tgt, tgt->data_rgb, src, SImage_subpixelRGB, tr, tr_inv);
    return;
  case SFmt_SeparateRGB:
    divTrGray(tgt, SImage_dataRed(tgt),
      src, SImage_subpixelRed,   tr, tr_inv);
    divTrGray(tgt, SImage_dataGreen(tgt),
      src, SImage_subpixelGreen, tr, tr_inv);
    divTrGray(tgt, SImage_dataBlue(tgt),
