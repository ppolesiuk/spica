  ...
  const STransform_t *tr_inv)
{
  ...
  for (int y = f.min_y; y < f.max_y; y++) {
    for (int x = f.min_x; x < f.max_x; x++) {
      ...
      tgt_data[y * f.tgt_w + x] *= spix[0] / spix[1];
    }
  }
}

static void maskTrRGB(
  SImage_t           *tgt,
  SVec4f_t           *tgt_data,
  const SImage_t     *src,
  subpixelGray_t      subpixel,
  ...
  const SImage_t     *src)
{
  ...
  case SFmt_Invalid:
    return;
  case SFmt_Gray:
    maskTrGray(tgt, tgt->data_gray, src, SImage_subpixelGray, tr, tr_inv);
    return;
  case SFmt_RGB:
    maskTrRGB(tgt, tgt->data_rgb, src, SImage_subpixelGray, tr, tr_inv);
    return;
  case SFmt_SeparateRGB:
    maskTrGray(tgt, SImage_dataRed(tgt),
      src, SImage_subpixelRed,   tr, tr_inv);
    maskTrGray(tgt, SImage_dataGreen(tgt),
      src, SImage_subpixelGreen, tr, tr_inv);
    maskTrGray(tgt, SImage_dataBlue(tgt),
  ...
  const SImage_t     *mask)
{
  ...
  maskTrMain(image, tr, &tr_inv, mask);
}
  ...
  const SImage_t     *mask)
{
  ...
  STransform_t tr_inv = STransform_inverse(tr);
  maskTrMain(image, &tr_inv, tr, mask);
}
